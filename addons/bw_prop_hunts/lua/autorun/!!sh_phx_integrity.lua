...
local CheckCVar = CreateConVar( "phx_integrity_check", "1", FCVAR_REPLICATED + FCVAR_ARCHIVE + FCVAR_NOTIFY, "Включить проверку целостности PH:X.", 0, 1 )
local cvIgnoreFretta = CreateConVar( "phx_integrity_check_fretta", "1", FCVAR_REPLICATED + FCVAR_ARCHIVE + FCVAR_NOTIFY, "Включить проверку установки другой базовой игры fretta", 0, 1 )
...
local function PHX___openWarningDialog( bFoundError, tResult )
	...
	form.window:SetTitle("PH:X Предупреждение: Обнаружен конфликт!")
	...
	if Errors > 0 then
		...
		Err = string.format("Предупреждение проверки целостности PH:X: PH:X установлен неправильно! Обнаружено ошибок: %d!", Errors)
		...
		for i,v in ipairs(ErrorList) do
			...
			form["labelErr"..i]:SetText("Обнаружена ошибка: "..v)
	...
	if (bFoundError) then
		...
		form.labelAddon:SetText("Обнаружены конфликты аддонов: (необходимо ОТКЛЮЧИТЬ/УДАЛИТЬ и ПЕРЕЗАПУСТИТЬ ИГРУ!)")
		...
		for k,v in pairs(tResult) do
			...
			form["labelAddErr_"..k]:SetText( string.format( "ID мастерской [%s]: '%s' (%s)", wsid, name, (isloaded and "Включено" or "Отключено") ) )
			...
			form["url_"..k]:SetText( "Нажмите здесь, чтобы отписаться от аддона '"..name.."'!" )
